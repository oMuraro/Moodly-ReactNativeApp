...
export default function DespertaCorpoScreen({ navigation }) {
  const [fase, setFase] = useState(0); // 0: preparar, 1: posição 1, 2: posição 2, 3: posição 3
  const [contador, setContador] = useState(5);
  const [ativo, setAtivo] = useState(false);
  const [ciclos, setCiclos] = useState(0);

  const fases = ['Prepare-se', 'Posição 1', 'Posição 2', 'Posição 3'];
  const instrucoes = [
    'Prepare-se para o alongamento',
    'Sentado na cadeira, levante os braços',
    'No chão, alongue as pernas',
    'Em pé, estique-se para cima'
  ];
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.title}>Desperta Corpo</Text>
        ...
        <View style={styles.circleContainer}>
          ...
          <View style={[styles.circle, styles.circle2, { 
            ...
            <Icon name="activity" size={30} color="#fff" />
          </View>
          <View style={styles.line} />
