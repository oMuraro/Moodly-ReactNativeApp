...
export default function ChatbotScreen() {
  ...
  // Substitua completamente a função antiga por esta versão final
const generateBotResponseWithGemini = async (userMessage, conversationHistory) => {
  const API_URL = `https://generativelanguage.googleapis.com/v1/models/gemini-2.0-flash:generateContent?key=AIzaSyBx_fEdLCjxLwkjCW6tuu9YZxChkCAAA4U`;
  ...
  // A instrução do sistema vai DENTRO do array 'contents' como o primeiro item.
  const contents = [
    {
      role: 'user', // O 'system instruction' é enviado como o primeiro 'user' na conversa.
      parts: [{ text: systemInstruction }],
    },
    ...formattedHistory, // O histórico da conversa vem depois.
    {
      role: 'user',
  ...
  try {
    const response = await fetch(API_URL, {
      ...
      // E removemos o 'system_instruction' do corpo principal da requisição.
      body: JSON.stringify({
        contents: contents,
        generationConfig: {
          temperature: 0.7,
