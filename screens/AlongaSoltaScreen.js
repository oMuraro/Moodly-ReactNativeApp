...
export default function AlongaSoltaScreen({ navigation }) {
  const [fase, setFase] = useState(0); // 0: preparar, 1: posição 1, 2: posição 2, 3: posição 3
  const [contador, setContador] = useState(10);
  const [ativo, setAtivo] = useState(false);
  const [ciclos, setCiclos] = useState(0);

  const fases = ['Prepare-se', 'Alonga braços', 'Alonga pernas', 'Relaxa'];
  const instrucoes = [
    'Prepare-se para alongar e soltar as tensões',
    'Estique os braços para cima e respire',
    'Alongue as pernas uma de cada vez',
    'Relaxe todo o corpo e respire profundo'
  ];
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.title}>Alonga e Solta</Text>
